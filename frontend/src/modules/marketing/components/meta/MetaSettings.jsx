    ...
    <div className="space-y-6">
      <div className="card-base p-6 space-y-4 border-l-4 border-blue-600">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <span className={`px-3.5 py-1.5 text-xs font-black rounded-md border shadow-sm ${statusInfo.badgeClass}`}>
            {statusInfo.label}
          </span>
        </div>

        {/* Status Callout Banner */}
        <div className={`p-4 rounded-md border text-xs space-y-2 flex items-start gap-3 ${statusInfo.badgeClass}`}>
          ...
          {(currentStatus === 'TOKEN_EXPIRED' || currentStatus === 'REAUTH_REQUIRED' || currentStatus === 'TOKEN_EXPIRING' || currentStatus === 'DISCONNECTED') && (
            <button
              ...
              className={`px-4 py-2 text-white font-black rounded-md text-xs shadow-md transition-all shrink-0 ${statusInfo.buttonClass}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
          <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-md space-y-2 border border-slate-200 dark:border-slate-700">
            ...
            <button
              onClick={handleConnectMetaAccount}
              className="mt-2 w-full py-2 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-md text-xs shadow-md transition-all flex items-center justify-center gap-1.5"
            >
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-md space-y-2 border border-slate-200 dark:border-slate-700">
            ...
            <div className="flex gap-2 mt-2">
              <button
                ...
                className="flex-1 py-2 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-md text-xs transition-all"
              >
                🔑 Update Access Token
              </button>
              <button
                ...
                disabled={isSyncing}
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-md text-xs shadow-md flex items-center gap-1"
              >
