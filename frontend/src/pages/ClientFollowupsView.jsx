...
function FollowupFormModal({ open, onClose, onSave, initial = null }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-white dark:bg-slate-900 rounded-md shadow-2xl w-full max-w-xl border border-slate-200 dark:border-slate-700 my-auto max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={form.company} onChange={e => set('company', e.target.value)}
                placeholder="e.g. Morya hospital" className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium" />
            </div>
            <div>
              ...
              <input type="text" value={form.tagline} onChange={e => set('tagline', e.target.value)}
                placeholder="e.g. health care services" className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              ...
              <input type="text" value={form.contactPerson} onChange={e => set('contactPerson', e.target.value)}
                placeholder="e.g. Manager" className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium" />
            </div>
            <div>
              <label className="text-[11px] font-extrabold text-slate-500 uppercase tracking-wider mb-1 block">Email</label>
              <input type="email" value={form.email} onChange={e => set('email', e.target.value)}
                placeholder="info@codigix.com" className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium" />
            </div>
            <div>
              <label className="text-[11px] font-extrabold text-slate-500 uppercase tracking-wider mb-1 block">Phone</label>
              <input type="text" value={form.phone} onChange={e => set('phone', e.target.value)}
                placeholder="+91 98765 00000" className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 font-medium" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              ...
              <input type="text" value={form.expectedValue} onChange={e => set('expectedValue', e.target.value)}
                placeholder="₹20,000" className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 font-bold" />
            </div>
            <div>
            ...
            <div>
              ...
              <select value={form.priority} onChange={e => set('priority', e.target.value)}
                className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-500 font-extrabold">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={form.industry} onChange={e => set('industry', e.target.value)}
                className="w-full px-3 py-2 text-sm bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none font-medium">
        ...
        <div className="flex items-center justify-end gap-3 p-4 sm:p-5 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-800/30">
          <button onClick={onClose} className="px-4 py-2 border border-slate-200 dark:border-slate-700 rounded-md text-xs font-extrabold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800">
            Cancel
          </button>
          <button onClick={handleSave} className="px-5 py-2 bg-brand-600 hover:bg-brand-700 text-white text-xs font-extrabold rounded-md shadow-md transition-all">
  ...
  return (
    <div className="space-y-4 sm:space-y-5 pb-20 lg:pb-12 text-slate-800 dark:text-slate-100">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="w-10 h-10 sm:w-auto sm:px-4 sm:py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-md sm:rounded-md text-xs font-extrabold flex items-center justify-center gap-1.5 hover:bg-slate-50 transition-all shadow-sm cursor-pointer"
          ...
          <button
            onClick={onOpenAI}
            className="w-10 h-10 sm:w-auto sm:px-4 sm:py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md sm:rounded-md text-xs font-extrabold flex items-center justify-center gap-1.5 shadow-md shadow-blue-500/20 transition-all cursor-pointer"
      ...
      <div className="grid grid-cols-4 gap-2 sm:gap-4">
        ...
        <div className="card-base p-2.5 sm:p-4 rounded-md border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex flex-col items-center sm:items-start text-center sm:text-left justify-between">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-md bg-emerald-500/10 text-emerald-600 flex items-center justify-center shrink-0 mb-1">
        ...
        <div className="card-base p-2.5 sm:p-4 rounded-md border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex flex-col items-center sm:items-start text-center sm:text-left justify-between">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-md bg-rose-500/10 text-rose-500 flex items-center justify-center shrink-0 mb-1">
        ...
        <div className="card-base p-2.5 sm:p-4 rounded-md border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm flex flex-col items-center sm:items-start text-center sm:text-left justify-between">
          <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-md bg-amber-500/10 text-amber-500 flex items-center justify-center shrink-0 mb-1">
      ...
      <div className="hidden sm:flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-slate-900 p-2.5 rounded-md border border-slate-200/80 dark:border-slate-800 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-md text-xs font-extrabold border flex items-center gap-1.5 transition-all cursor-pointer ${activeTab === 'Starred'
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-3 py-1.5 text-xs font-bold bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md text-slate-700 dark:text-slate-200 focus:outline-none cursor-pointer"
          >
          ...
          <div className="relative w-56 lg:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-md focus:outline-none text-slate-800 dark:text-slate-200 font-medium"
            />
            {searchQuery && (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
        ...
        <div className="lg:col-span-8 space-y-4">
          ...
          <div className="hidden lg:block">
            <DataTable
              ...
              actionButton={
                <button
                  onClick={() => { setEditClient(null); setShowModal(true); }}
                  className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-md flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
                >
            ...
            {filteredClients.map(client => {
              ...
              return (
                <div
                  ...
                  className={`p-3.5 rounded-md border transition-all cursor-pointer bg-white dark:bg-slate-900 shadow-sm ${isSelected
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="w-9 h-9 rounded-md bg-blue-600 text-white font-black flex items-center justify-center text-xs shrink-0">
                        {client.company ? client.company[0].toUpperCase() : 'C'}
                      </div>
                      <div className="min-w-0">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-md border border-slate-200/80 dark:border-slate-800 p-4 sm:p-5 shadow-sm space-y-3">
          ...
          <div className="bg-purple-50/60 dark:bg-purple-950/30 border border-purple-100 dark:border-purple-900/50 p-4 rounded-md space-y-2 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
            <div>
            ...
            <button onClick={onOpenAI} className="px-3 py-1.5 bg-purple-100 dark:bg-purple-900/60 hover:bg-purple-200 text-purple-800 dark:text-purple-200 text-xs font-extrabold rounded-md shrink-0 cursor-pointer self-start sm:self-auto transition-all">
              View Insights
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          {selectedClient ? (
            <>
              ...
              <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white rounded-md p-4 sm:p-5 flex items-center justify-between shadow-md">
                <div>
              ...
              <div className="bg-white dark:bg-slate-900 rounded-md border border-slate-200/80 dark:border-slate-800 p-4 sm:p-5 shadow-sm space-y-4">
                <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-3">
                  <div className="w-10 h-10 rounded-md bg-blue-50 dark:bg-blue-950/50 text-blue-600 font-black flex items-center justify-center text-sm shrink-0">
                ...
                <div className="space-y-2.5 text-xs font-semibold text-slate-700 dark:text-slate-200">
                  ...
                  <div className="grid grid-cols-4 gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                    <button onClick={() => alert(`Starting chat with ${selectedClient.company}`)} className="p-2 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 rounded-md text-slate-600 dark:text-slate-300 flex items-center justify-center cursor-pointer">
                      <MessageSquare className="w-4 h-4" />
                    </button>
                    <button onClick={() => alert(`Calling ${selectedClient.phone || '+91 98765 00000'}`)} className="p-2 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 rounded-md text-slate-600 dark:text-slate-300 flex items-center justify-center cursor-pointer">
                      <Phone className="w-4 h-4" />
                    </button>
                    <button onClick={() => alert(`Sending email to ${selectedClient.email || 'info@codigix.com'}`)} className="p-2 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 rounded-md text-slate-600 dark:text-slate-300 flex items-center justify-center cursor-pointer">
                      <Mail className="w-4 h-4" />
                    </button>
                    <button onClick={() => { setEditClient(selectedClient); setShowModal(true); }} className="p-2 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 rounded-md text-slate-600 dark:text-slate-300 flex items-center justify-center cursor-pointer">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-md border border-slate-200/80 dark:border-slate-800 p-8 text-center space-y-2 shadow-sm">
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-md shadow-2xl border border-slate-200 dark:border-slate-800 p-6 w-full max-w-sm space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-rose-100 dark:bg-rose-900/40 rounded-md flex items-center justify-center shrink-0">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 p-3 rounded-md font-bold truncate">
              {clients.find(c => c.id === confirmDeleteId)?.company}
            </p>
            <div className="flex gap-2 justify-end">
              <button onClick={() => setConfirmDeleteId(null)}
                className="px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md">Cancel</button>
              <button onClick={() => handleDeleteClient(confirmDeleteId)}
                className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-md shadow-sm">Delete</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl sm:rounded-3xl shadow-2xl w-full max-w-lg p-5 space-y-4 max-h-[85vh] overflow-y-auto no-scrollbar">
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="space-y-1.5">
                ...
                <button
                  onClick={() => setActiveTab(activeTab === 'Starred' ? 'All' : 'Starred')}
                  className={`w-full py-2 px-3 rounded-md text-xs font-bold border flex items-center justify-between cursor-pointer transition-all ${activeTab === 'Starred'
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['All Priority', 'High', 'Medium', 'Low'].map((p) => (
                    <button
                      ...
                      className={`py-2 px-2 rounded-md text-xs font-bold border text-center cursor-pointer transition-all truncate ${priorityFilter === p
                ...
                <div className="grid grid-cols-2 gap-2">
                  {['All Status', ...STATUSES].map((s) => (
                    <button
                      ...
                      className={`py-2 px-3 rounded-md text-xs font-bold border text-left cursor-pointer transition-all truncate ${statusFilter === s
                        ? 'bg-blue-600 text-white border-blue-600'
                        : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300'
                        }`}
                    >
